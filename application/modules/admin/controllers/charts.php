...
class Charts extends admin {
	...
	function addition_totals($timestamp)
	{
		...
		$total = $this->reports_model->get_addition_total(1, $date);
		...
		$total = $this->reports_model->get_addition_total(0, $date);
		//mark the highest bar
		if($total > $highest_bar)
		{
			$highest_bar = $total;
	...
	function click_totals()
	{
		...
		if($parents->num_rows() > 0)
		{
			...
			foreach($result as $res)
			{
				...
				//get service total
				$total = $this->reports_model->get_click_totals($category_id);
	...
	function product_totals()
	{
		...
		if($parents->num_rows() > 0)
		{
			...
			foreach($result as $res)
			{
				...
				//get service total
				$total = $this->reports_model->get_product_totals($category_id);
